...
export type FrontmatterStore<T extends MarkdownFrontmatter = MarkdownFrontmatter> = Readable<T>;

export const frontmatter: FrontmatterStore = toStore(useFrontmatter);

export type StaticDataStore<T extends StaticLoader | LoadedStaticData = LoadedStaticData> =
  Readable<InferStaticLoaderData<T>>;

export const staticData: StaticDataStore = toStore(useStaticData);

export type ServerDataStore<T extends ServerLoader | LoadedServerData = LoadedServerData> =
  Readable<InferServerLoaderData<T>>;

export const serverData: ServerDataStore = toStore(useServerData);

export type ServerErrorStore<T extends HttpErrorData = HttpErrorData> = Readable<HttpError<T>>;
