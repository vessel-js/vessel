...
import { createServerRouter, type ServerApp, type ServerRouter } from './server-router';

export function configureServer(init: (app: { app: ServerApp; router: ServerRouter }) => void) {
...
function installServerConfig(manifest: ServerManifest, config: ServerConfig) {
  ...
  for (const type of ['page', 'api'] as const) {
    ...
    if (config.errorHandlers[type].length > 0) {
      manifest.errorHandlers[type]!.push(...config.errorHandlers[type].map(addURLPattern));
...
export function addURLPattern<T extends { pathname: string; pattern?: URLPattern }>(route: T) {
  if (!route.pattern) {
