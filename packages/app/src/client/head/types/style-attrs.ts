/**
 * Credit: https://github.com/harlan-zw/zhead/blob/main/packages/schema/src/style.ts
 */
...
export interface StyleTagAttributes {
  ...
  media?: string;
  /**
   * A cryptographic nonce (number used once) used to allow inline styles in a style-src
   * Content-Security-Policy. The server must generate a unique nonce value each time it transmits
   * a policy. It is critical to provide a nonce that cannot be guessed as bypassing a resource's
