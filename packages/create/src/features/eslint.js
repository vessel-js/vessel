...
export function addEslintFeature(builder) {
  ...
  builder.pkg.addDep('eslint', '^7.0.0', { dev: true });
  builder.pkg.addDep('eslint-plugin-import', '^2.0.0', { dev: true });
  builder.pkg.addDep('eslint-plugin-simple-import-sort', '^7.0.0', {
    dev: true,
  });
  ...
  if (builder.hasFeature('prettier')) {
    builder.pkg.addDep('eslint-config-prettier', '^8.0.0', {
      dev: true,
    });
  }

  if (builder.hasFeature('typescript')) {
    builder.pkg.addDep('@typescript-eslint/eslint-plugin', '^4.0.0', {
      dev: true,
    });
    builder.pkg.addDep('@typescript-eslint/parser', '^4.0.0', {
      dev: true,
    });
    builder.pkg.addDep('eslint-import-resolver-typescript', '^2.5.0', {
      dev: true,
    });
  }
  ...
  if (builder.framework === 'svelte') {
    builder.pkg.addDep('eslint-plugin-svelte3', '^3.0.0', {
      dev: true,
    });
  }
