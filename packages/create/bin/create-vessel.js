...
async function main() {
  ...
  const projectPkgName = toValidPackageName(projectDirName ?? 'vessel-app');
  ...
  builder.pkg.addDep('vite', '^3.0.0', { dev: true });

  // Svelte
  builder.pkg.addDep('svelte', '^3.40.0', { dev: true });
  builder.pkg.addDep('@sveltejs/vite-plugin-svelte', '^1.0.0', {
    dev: true,
  });
